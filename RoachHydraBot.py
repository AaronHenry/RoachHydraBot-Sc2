...
class ROACHHYDRA(sc2.BotAI):
    ...
    async def on_step(self, iteration):
        ...
        if self.units(ROACH).amount > 18 and self.units(HYDRALISK).amount > 12 and iteration % 50 == 0:
        ...
        if self.structures(ROACHWARREN).ready.exists:
            if self.can_afford(ROACH) and larvae.exists:
                self.do(larvae.random.train(ROACH))
                return
        ...
        if self.structures(SPAWNINGPOOL).ready.exists:
         ...
         if self.townhalls(SPAWNINGPOOL).ready.exists:
            if not (self.structures(ROACHWARREN).exists or self.already_pending(ROACHWARREN)):
...
def main():
    sc2.run_game(
        ...
        [Bot(Race.Zerg, ROACHHYDRA()), Computer(Race.Terran, Difficulty.Medium)],
        realtime=False,
